...
public class TimeOfDayConditionHandler extends BaseConditionModuleHandler implements TimeBasedConditionHandler {
    ...
    public boolean isSatisfiedAt(ZonedDateTime time) {
        LocalTime localStartTime = startTime;
        LocalTime localEndTime = endTime;
        ...
        if (localStartTime.isBefore(localEndTime)) {
            if (currentTime.isAfter(localStartTime) && currentTime.isBefore(localEndTime)) {
                logger.debug("Time condition with id {} evaluated, that {} is between {} and {}.", module.getId(),
                        currentTime, localStartTime, localEndTime);
                return true;
            }
        }
        ...
        else if (currentTime.isAfter(LocalTime.MIDNIGHT) && currentTime.isBefore(localEndTime)
                || currentTime.isAfter(localStartTime) && currentTime.isBefore(LocalTime.MAX)) {
            logger.debug("Time condition with id {} evaluated, that {} is between {} and {}, or between {} and {}.",
                    module.getId(), currentTime, LocalTime.MIDNIGHT, localEndTime, localStartTime,
